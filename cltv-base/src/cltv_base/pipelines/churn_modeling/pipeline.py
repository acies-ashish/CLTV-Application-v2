# src/cltv_base/pipelines/churn_modeling/pipeline.py
...
def create_pipeline(**kwargs) -> Pipeline:
    """
    Creates a pipeline for churn modeling, including identifying at-risk customers,
    training a churn prediction model, and a Cox survival model.
    """
    return Pipeline(
        [
            node(
                func=get_customers_at_risk,
                inputs=["historical_cltv_customers", "params:at_risk_threshold_days"],
            ...
            node(
                ...
                name="get_churn_features_and_labels",
            ),
            node(
                func=train_churn_prediction_model,
                inputs=["churn_features", "churn_labels", "params:churn_model_n_estimators", "params:churn_model_random_state"],
                outputs=["churn_prediction_model", "churn_classification_report", "churn_feature_importances", "churn_features_test", "churn_labels_test"],
                name="train_churn_prediction_model",
            ),
            node(
                func=predict_churn_probabilities,
                inputs=["churn_prediction_model", "churn_features"], # Predict on full features for dashboard display
        ...
        tags="churn_modeling" # Optional: Add a tag for this pipeline
    )
